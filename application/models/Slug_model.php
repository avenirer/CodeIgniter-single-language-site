...
class Slug_model extends MY_Model
{
    ...
    {
        $the_new_slug = $this->_verify_slug($insert_data['url']);
        $insert_data['url'] = $the_new_slug;
        if($slug_id = $this->insert($insert_data))
        {
            $this->where(array('content_type'=>$insert_data['content_type'], 'id !='=>$slug_id))->update(array('redirect'=>$slug_id));
            return TRUE;
        }
        return FALSE;
    }
    ...
    {
        if($this->where(array('url'=>$str))->get() !== FALSE)
        {
            $parts = explode('-',$str);
