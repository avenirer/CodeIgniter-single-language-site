...
class Menus {
    ...
    public function get_menu($menu_name, $return = 'array')
    {
        ...
        if(!empty($menus))
        {
            ...
            if(!empty($menu_items))
            {
                ...
                {
                    $url = ($item->absolute_path == '1') ? $item->url :  site_url($item->url);
                    $the_menu[] = array('id'=>$item->id, 'parent_id'=>$item->parent_id, 'title'=>$item->title, 'url'=>$url);
                }
            }
